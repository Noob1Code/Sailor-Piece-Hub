...
local Theme = {
    Background = Color3.fromRGB(20, 20, 25), Sidebar = Color3.fromRGB(28, 28, 35), Component = Color3.fromRGB(35, 35, 45),
    Accent = Color3.fromRGB(0, 195, 255), Text = Color3.fromRGB(255, 255, 255), TextDim = Color3.fromRGB(160, 160, 170),
...
function Interface:Notify(title, text, duration)
    ...
    Notif.Size = UDim2.new(1, 0, 0, 65); Notif.BackgroundColor3 = Theme.Component; Notif.BackgroundTransparency = 1 
    Instance.new("UICorner", Notif).CornerRadius = UDim.new(0, 6)
    
    local SideBar = Instance.new("Frame")
    SideBar.Size = UDim2.new(0, 4, 1, 0); SideBar.BackgroundColor3 = Theme.Accent; SideBar.BorderSizePixel = 0
    SideBar.BackgroundTransparency = 1; SideBar.Parent = Notif
    ...
    TitleLbl.TextTransparency = 1; TitleLbl.Font = Enum.Font.GothamBold; TitleLbl.TextSize = 13
    TitleLbl.TextXAlignment = Enum.TextXAlignment.Left; TitleLbl.Parent = Notif

    local DescLbl = Instance.new("TextLabel")
    DescLbl.Size = UDim2.new(1, -20, 0, 35); DescLbl.Position = UDim2.new(0, 15, 0, 25)
    DescLbl.BackgroundTransparency = 1; DescLbl.Text = text; DescLbl.TextColor3 = Theme.TextDim
    DescLbl.TextTransparency = 1; DescLbl.Font = Enum.Font.Gotham; DescLbl.TextSize = 12
    DescLbl.TextWrapped = true; DescLbl.TextXAlignment = Enum.TextXAlignment.Left; DescLbl.Parent = Notif
...
function Interface:CreateTab(name, icon)
    ...
    TabBtn.Size = UDim2.new(1, 0, 0, 40); TabBtn.BackgroundColor3 = Theme.Background; TabBtn.BorderSizePixel = 0
    TabBtn.TextColor3 = Theme.TextDim; TabBtn.Text = "  " .. (icon or "") .. " " .. name
    TabBtn.Font = Enum.Font.GothamSemibold; TabBtn.TextSize = 13; TabBtn.TextXAlignment = Enum.TextXAlignment.Left
    ...
    ContentLayout.Parent = TabContent; ContentLayout.SortOrder = Enum.SortOrder.LayoutOrder; ContentLayout.Padding = UDim.new(0, 10) 
    ...
    table.insert(self.Connections, TabBtn.MouseButton1Click:Connect(function()
        for _, tabInfo in pairs(self.Tabs) do tabInfo.Content.Visible = false; TweenService:Create(tabInfo.Button, TweenInfo.new(0.3), {BackgroundColor3 = Theme.Background, TextColor3 = Theme.TextDim}):Play() end
        TabContent.Visible = true; self.CurrentTab = TabContent; TweenService:Create(TabBtn, TweenInfo.new(0.3), {BackgroundColor3 = Theme.Component, TextColor3 = Theme.Accent}):Play()
    end))

    if not self.CurrentTab then TabContent.Visible = true; TabBtn.BackgroundColor3 = Theme.Component; TabBtn.TextColor3 = Theme.Accent; self.CurrentTab = TabContent end
    table.insert(self.Tabs, {Button = TabBtn, Content = TabContent}); Tab.Container = TabContent

    function Tab:CreateLabel(text)
        local Label = Instance.new("TextLabel"); Label.Size = UDim2.new(1, 0, 0, 20); Label.BackgroundTransparency = 1; Label.TextColor3 = Theme.Accent
    ...
    function Tab:CreateButton(text, callback, customColor)
        local color = customColor or Theme.Sidebar; local Btn = Instance.new("TextButton")
        Btn.Size = UDim2.new(1, -10, 0, 35); Btn.BackgroundColor3 = color; Btn.TextColor3 = Theme.Text; Btn.Text = text
    ...
    function Tab:CreateToggle(text, defaultState, callback)
        local frame = Instance.new("Frame"); frame.Size = UDim2.new(1, -10, 0, 38); frame.BackgroundColor3 = Theme.Component; frame.Parent = self.Container
        Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6)
        local label = Instance.new("TextLabel"); label.Size = UDim2.new(1, -60, 1, 0); label.Position = UDim2.new(0, 15, 0, 0)
        label.BackgroundTransparency = 1; label.Text = text; label.TextColor3 = Theme.Text; label.Font = Enum.Font.Gotham; label.TextSize = 13; label.TextXAlignment = Enum.TextXAlignment.Left; label.Parent = frame
        local btn = Instance.new("TextButton"); btn.Size = UDim2.new(0, 44, 0, 22); btn.Position = UDim2.new(1, -55, 0.5, -11)
        btn.BackgroundColor3 = Theme.Background; btn.Text = ""; btn.Parent = frame; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 11)
        local indicator = Instance.new("Frame"); indicator.Size = UDim2.new(0, 18, 0, 18); local state = defaultState and true or false
        indicator.Position = state and UDim2.new(1, -20, 0.5, -9) or UDim2.new(0, 2, 0.5, -9); indicator.BackgroundColor3 = state and Theme.Green or Theme.Red; indicator.Parent = btn; Instance.new("UICorner", indicator).CornerRadius = UDim.new(0, 9)

        table.insert(Tab.Window.Connections, btn.MouseButton1Click:Connect(function()
            state = not state
            if state then TweenService:Create(indicator, TweenInfo.new(0.2), {Position = UDim2.new(1, -20, 0.5, -9), BackgroundColor3 = Theme.Green}):Play()
            else TweenService:Create(indicator, TweenInfo.new(0.2), {Position = UDim2.new(0, 2, 0.5, -9), BackgroundColor3 = Theme.Red}):Play() end
            callback(state); if typeof(Tab.Window.Config.Save) == "function" then Tab.Window.Config:Save() end
        end))
    end
    ...
    function Tab:CreateDropdown(title, options, defaultOption, callback)
        ...
        local DropFrame = Instance.new("Frame"); DropFrame.Size = UDim2.new(1, -10, 0, 40); DropFrame.BackgroundColor3 = Theme.Component
        DropFrame.ClipsDescendants = true; DropFrame.Parent = self.Container; Instance.new("UICorner", DropFrame).CornerRadius = UDim.new(0, 6)
        
        local MainBtn = Instance.new("TextButton"); MainBtn.Size = UDim2.new(1, 0, 0, 40); MainBtn.BackgroundTransparency = 1; MainBtn.TextColor3 = Theme.Text
        MainBtn.Text = "  " .. title .. ": " .. tostring(defaultOption); MainBtn.Font = Enum.Font.GothamSemibold; MainBtn.TextSize = 13; MainBtn.TextXAlignment = Enum.TextXAlignment.Left; MainBtn.Parent = DropFrame
        local Arrow = Instance.new("TextLabel"); Arrow.Size = UDim2.new(0, 30, 1, 0); Arrow.Position = UDim2.new(1, -30, 0, 0)
        Arrow.BackgroundTransparency = 1; Arrow.Text = "▼"; Arrow.TextColor3 = Theme.TextDim; Arrow.Font = Enum.Font.GothamBold; Arrow.Parent = MainBtn

        local ListFrame = Instance.new("ScrollingFrame"); ListFrame.Size = UDim2.new(1, 0, 1, -40); ListFrame.Position = UDim2.new(0, 0, 0, 40)
        ...
        local function updateSelection(opt)
            ...
            TweenService:Create(DropFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, -10, 0, 40)}):Play(); Arrow.Text = "▼"
        ...
        table.insert(Tab.Window.Connections, MainBtn.MouseButton1Click:Connect(function()
            isOpen = not isOpen; local targetHeight = isOpen and math.min(ListLayout.AbsoluteContentSize.Y + 40, 160) or 40
            TweenService:Create(DropFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, -10, 0, targetHeight)}):Play(); Arrow.Text = isOpen and "▲" or "▼"
        ...
        local function refresh(newOptions)
            ...
            for _, opt in ipairs(newOptions) do
                local OptBtn = Instance.new("TextButton"); OptBtn.Size = UDim2.new(1, 0, 0, 30); OptBtn.BackgroundColor3 = Theme.Sidebar
                OptBtn.TextColor3 = Theme.TextDim; OptBtn.Text = "  " .. tostring(opt); OptBtn.Font = Enum.Font.Gotham; OptBtn.TextSize = 13
                OptBtn.TextXAlignment = Enum.TextXAlignment.Left; OptBtn.Parent = ListFrame; OptBtn.AutoButtonColor = false
    ...
    function Tab:CreateTextBox(title, defaultText, callback)
        local Container = Instance.new("Frame"); Container.Size = UDim2.new(1, -10, 0, 40); Container.BackgroundColor3 = Theme.Component; Container.Parent = self.Container
        Instance.new("UICorner", Container).CornerRadius = UDim.new(0, 6)
        local Label = Instance.new("TextLabel"); Label.Size = UDim2.new(0.6, 0, 1, 0); Label.Position = UDim2.new(0, 15, 0, 0)
        Label.BackgroundTransparency = 1; Label.TextColor3 = Theme.Text; Label.Text = "  " .. title; Label.Font = Enum.Font.GothamSemibold; Label.TextSize = 13; Label.TextXAlignment = Enum.TextXAlignment.Left; Label.Parent = Container
        local TextBox = Instance.new("TextBox"); TextBox.Size = UDim2.new(0.35, -5, 0.7, 0); TextBox.Position = UDim2.new(0.65, 0, 0.15, 0)
        TextBox.BackgroundColor3 = Theme.Background; TextBox.TextColor3 = Theme.Accent; TextBox.Text = tostring(defaultText)
        TextBox.Font = Enum.Font.Gotham; TextBox.TextSize = 13; TextBox.Parent = Container; Instance.new("UICorner", TextBox).CornerRadius = UDim.new(0, 4)
        table.insert(Tab.Window.Connections, TextBox.FocusLost:Connect(function() 
            local val = TextBox.Text; if tonumber(val) then callback(tonumber(val)) else callback(val) end; if typeof(Tab.Window.Config.Save) == "function" then Tab.Window.Config:Save() end
        end))
    end
    return Tab
end
...
function Interface:BuildUI()
    ...
    self.MainFrame = Instance.new("Frame"); self.MainFrame.Size = UDim2.new(0, 580, 0, 480); self.MainFrame.Position = UDim2.new(0.5, -290, 0.5, -240)
    self.MainFrame.BackgroundColor3 = Theme.Background; self.MainFrame.ClipsDescendants = true; self.MainFrame.Parent = self.ScreenGui
    Instance.new("UICorner", self.MainFrame).CornerRadius = UDim.new(0, 8)
    
    local TitleBar = Instance.new("TextButton"); TitleBar.Size = UDim2.new(1, 0, 0, 45); TitleBar.BackgroundColor3 = Theme.Sidebar; TitleBar.TextColor3 = Theme.Accent
    TitleBar.Text = "   🌟 Sailor Piece Hub V2"; TitleBar.Font = Enum.Font.GothamBold; TitleBar.TextSize = 16; TitleBar.TextXAlignment = Enum.TextXAlignment.Left
    TitleBar.Parent = self.MainFrame; TitleBar.AutoButtonColor = false; Instance.new("UICorner", TitleBar).CornerRadius = UDim.new(0, 8)
    
    local dragging, dragInput, dragStart, startPos
    table.insert(self.Connections, TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; dragStart = input.Position; startPos = self.MainFrame.Position; input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false end end) end
    end))
    ...
    self.TabSelector = Instance.new("ScrollingFrame"); self.TabSelector.Size = UDim2.new(0, 160, 1, -45); self.TabSelector.Position = UDim2.new(0, 0, 0, 45)
    self.TabSelector.BackgroundColor3 = Theme.Background; self.TabSelector.BorderSizePixel = 0; self.TabSelector.ScrollBarThickness = 2; self.TabSelector.Parent = self.MainFrame
    Instance.new("UIListLayout", self.TabSelector).Padding = UDim.new(0, 2)
    
    self.ContentContainer = Instance.new("Frame"); self.ContentContainer.Size = UDim2.new(1, -160, 1, -45); self.ContentContainer.Position = UDim2.new(0, 160, 0, 45)
    self.ContentContainer.BackgroundTransparency = 1; self.ContentContainer.Parent = self.MainFrame

    local CloseBtn = Instance.new("TextButton"); CloseBtn.Size = UDim2.new(0, 30, 0, 30); CloseBtn.Position = UDim2.new(1, -40, 0.5, -15)
    CloseBtn.BackgroundTransparency = 1; CloseBtn.TextColor3 = Theme.Red; CloseBtn.Text = "✖"; CloseBtn.Font = Enum.Font.GothamBold; CloseBtn.TextSize = 16; CloseBtn.Parent = TitleBar
    table.insert(self.Connections, CloseBtn.MouseButton1Click:Connect(function() if typeof(_G.ComunidadeHub_Cleanup) == "function" then _G.ComunidadeHub_Cleanup() end end))

    self.NotifyFrame = Instance.new("Frame"); self.NotifyFrame.Size = UDim2.new(0, 250, 1, -20); self.NotifyFrame.Position = UDim2.new(1, -270, 0, 10)
    self.NotifyFrame.BackgroundTransparency = 1; self.NotifyFrame.Parent = self.ScreenGui
    ...
    local function GetMobsFromConstants(island)
        ...
        if island == "Todas" then for _, data in pairs(self.Constants.IslandDataMap) do if data.Mobs then for _, m in ipairs(data.Mobs) do table.insert(list, m) end end end
        else if self.Constants.IslandDataMap[island] and self.Constants.IslandDataMap[island].Mobs then for _, m in ipairs(self.Constants.IslandDataMap[island].Mobs) do table.insert(list, m) end end end
        return list
    end

    local function GetBossesFromConstants(island)
        local list = {"Nenhum"}
        if island == "Todas" then for _, data in pairs(self.Constants.IslandDataMap) do if data.Bosses then for _, b in ipairs(data.Bosses) do table.insert(list, b) end end end
        else if self.Constants.IslandDataMap[island] and self.Constants.IslandDataMap[island].Bosses then for _, b in ipairs(self.Constants.IslandDataMap[island].Bosses) do table.insert(list, b) end end end
        return list
    end

    local function GetQuestsFromConstants(island)
        local list = {"Nenhum"}
        if self.Constants.QuestDataMap[island] then for _, q in ipairs(self.Constants.QuestDataMap[island]) do table.insert(list, q.Name) end end
        return list
    end
    ...
    TabMissions:CreateDropdown("Ilha", self.Constants.QuestFilterOptions, self.Config.SelectedQuestIsland, function(s) self.Config.SelectedQuestIsland = s; if questDropdownRef then questDropdownRef.Refresh(GetQuestsFromConstants(s)) end end)
    ...
    -- ATUALIZAÇÃO DO FILTRO PARA O CONFIG: Assim o FSM sabe qual ilha você escolheu para "Todos"
    TabCombat:CreateDropdown("Filtro de Ilha", self.Constants.FilterOptions, self.Config.SelectedIslandFilter, function(s) 
        self.Config.SelectedIslandFilter = s 
    ...
    mobDropdownRef = TabCombat:CreateDropdown("Alvo (Mob)", GetMobsFromConstants(self.Config.SelectedIslandFilter), self.Config.SelectedMob, function(s) self.Config.SelectedMob = s end)
    ...
    bossDropdownRef = TabCombat:CreateDropdown("Alvo (Boss)", GetBossesFromConstants(self.Config.SelectedIslandFilter), self.Config.SelectedBoss, function(s) self.Config.SelectedBoss = s end)
    ...
    TabCombat:CreateButton("🗑️ Limpar Fila", function()
        self.Config.SelectedBosses = {}; BossListLabel.Text = "Fila: Nenhuma"; if self.Config.Save then self.Config:Save() end
    end, Theme.Red)
    ...
    self:Notify("Patch Aplicado", "Erros de posição e 'Todos' corrigidos!", 4)
end
